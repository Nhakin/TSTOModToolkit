...
Type
  ...
  ITSTORgbFile = Interface(IInterface)
    ...
    Procedure SaveRgbToStream(AStream : TStream);
    Procedure LoadRgbFromStream(AStream : TStream);
...
//  TRgbAPixels = Array Of Array Of Word;
  ...
  TTSTORgbFileImpl = Class(TInterfacedObject, ITSTORgbFile)
  Private
    FFileSign    : DWord;
  ...
  Protected
    ...
    Procedure SaveRgbToStream(AStream : TStream; APixels : TRgbA4Pixels); OverLoad;
    Procedure SaveRgbToStream(AStream : TStream); OverLoad;
    Procedure LoadRgbFromStream(AStream : TStream);
    Procedure LoadRgbFromFile(Const AFileName : String);
    Procedure LoadPngImage(APngImage : TPngImage);
    Procedure LoadPngFromFile(Const AFileName : String);
  ...
  TTSTORgbFiles = Class(TInterfaceList, ITSTORgbFiles)
  Protected
...
Function TTSTORgbFileImpl.GetInterfaceObject() : TObject;
Begin
  Result := Self;
End;
...
Procedure TTSTORgbFileImpl.SaveRgbToStream(AStream : TStream; APixels : TRgbA4Pixels);
Var X, Y : Integer;
    lHeader : DWord;
    lWord : Word;
Begin
  ...
  Begin
    lHeader := $20000000;
    AStream.WriteBuffer(lHeader, SizeOf(DWord));
    lWord := Length(APixels[0]);
    AStream.WriteBuffer(lWord, SizeOf(Word));
    lWord := Length(APixels);
    AStream.WriteBuffer(lWord, SizeOf(Word));

    For Y := Low(APixels) To High(APixels) Do
      For X := Low(APixels[Y]) To High(APixels[Y]) Do
        AStream.WriteBuffer(APixels[Y][X], SizeOf(Word));
  End;
End;

Procedure TTSTORgbFileImpl.SaveRgbToStream(AStream : TStream);
Begin
  SaveRgbToStream(AStream, FPixels);
End;

Procedure TTSTORgbFileImpl.LoadRgbFromStream(AStream : TStream);
Var lWidth  : Word;
    lHeight : Word;
    X, Y    : Integer;
Begin
  ...
  AStream.Position := 0;
  AStream.ReadBuffer(FFileSign, SizeOf(FFileSign));
  If (FFileSign = $20000000) Or (FFileSign = $60000000) Then
  Begin
    AStream.ReadBuffer(lWidth, SizeOf(lWidth));
    AStream.ReadBuffer(lHeight, SizeOf(lHeight));
    ...
    Begin
      ...
      For X := 0 To lWidth - 1 Do
        AStream.ReadBuffer(FPixels[Y][X], SizeOf(FPixels[Y][X]));
    End;
  End
  ...
  Begin
    ...
    Begin
      ...
      For X := 0 To lWidth - 1 Do
        AStream.ReadBuffer(FPixelsDword[Y][X], SizeOf(FPixelsDword[Y][X]));
    End;
  End
  Else
