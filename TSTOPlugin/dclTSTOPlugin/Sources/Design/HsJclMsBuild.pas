...
procedure Test;
var
  MsBuildParser: THsJclMsBuildParser;
begin
  MsBuildParser := THsJclMsBuildParser.Create('C:\dev\jcl\jcl\packages\d11\Jcl.dproj');
  try
    MsBuildParser.Init;
    MsBuildParser.Parse;
...
type
  ...
  EHsJclMsBuildError = class(EHsJclError);

  THsJclMsBuildItem = class
  private
    FItemName: string;
    FItemInclude: string;
  ...
  private
    ...
    FTaskBody: string;
    function AddParameter(Parameter: THsJclMsBuildParameter): Integer;
    function GetParameterCount: Integer;
    function GetParameter(Index: Integer): THsJclMsBuildParameter;
  public
    constructor Create;
    destructor Destroy; override;
    ...
    property Parameters[Index: Integer]: THsJclMsBuildParameter read GetParameter;
    property TaskBody: string read FTaskBody;
  end;

  THsJclMsBuildTask = class
  private
    FTaskName: string;
    FContinueOnError: Boolean;
    FParameters: TStrings;
    FOutputs: TObjectList;
    function AddOutput(AOutput: THsJclMsBuildTaskOutput): Integer;
    function GetOutputCount: Integer;
    function GetOutput(Index: Integer): THsJclMsBuildTaskOutput;
  public
    constructor Create;
    destructor Destroy; override;
    property TaskName: string read FTaskName;
    ...
    property Outputs[Index: Integer]: THsJclMsBuildTaskOutput read GetOutput;
  end;

  THsJclMsBuildTarget = class
  private
    FTargetName: string;
    FDepends: TStrings;
    FReturns: TStrings;
    ...
    function AddTask(Task: THsJclMsBuildTask): Integer;
    function GetTaskCount: Integer;
    function GetTask(Index: Integer): THsJclMsBuildTask;
  public
    constructor Create;
    destructor Destroy; override;
    ...
    property Tasks[Index: Integer]: THsJclMsBuildTask read GetTask;
    property ErrorTargets: TStrings read FErrorTargets;
  end;

  THsJclMsBuildParser = class;
  ...
  THsJclMsBuildProperties = class(TStrings)
  private
    FParser: THsJclMsBuildParser;
  ...
  public
    constructor Create(AParser: THsJclMsBuildParser);
    destructor Destroy; override;

    property Parser: THsJclMsBuildParser read FParser;
  ...
  THsJclMsBuildImportEvent = procedure (Sender: THsJclMsBuildParser; var FileName: TFileName;
    var SubXml: THsJclSimpleXml; var SubOwnsXml: Boolean) of object;
  THsJclMsBuildToolsVersionEvent = procedure (Sender: THsJclMsBuildParser; const ToolsVersion: string) of object;
  THsJclMsBuildRegistryPropertyEvent = function (Sender: THsJclMsBuildParser; Root: HKEY;
    const Path, Name: string; out Value: string): Boolean of object;
  THsJclMsBuildFunctionPropertyEvent = function (Sender: THsJclMsBuildParser; const Command: string;
  ...
  private
    ...
    FXml: THsJclSimpleXml;
    FOwnsXml: Boolean;
    FProperties: THsJclMsBuildProperties;
    FItems: TObjectList;
    ...
    FFirstPropertyGroup: THsJclSimpleXMLElem;
    FProjectExtensions: THsJclSimpleXMLElem;
    FOnImport: THsJclMsBuildImportEvent;
    FOnToolsVersion: THsJclMsBuildToolsVersionEvent;
    FOnRegistryProperty: THsJclMsBuildRegistryPropertyEvent;
    FOnFunctionProperty: THsJclMsBuildFunctionPropertyEvent;
    function GetItemCount: Integer;
    function GetItem(Index: Integer): THsJclMsBuildItem;
    function GetItemDefinitionCount: Integer;
    function GetItemDefinition(Index: Integer): THsJclMsBuildItem;
    function GetTargetCount: Integer;
    function GetTarget(Index: Integer): THsJclMsBuildTarget;
    function GetUsingTaskCount: Integer;
    function GetUsingTask(Index: Integer): THsJclMsBuildUsingTask;
    procedure ParseChoose(XmlElem: THsJclSimpleXmlElem);
    procedure ParseImport(XmlElem: THsJclSimpleXmlElem);
    procedure ParseImportGroup(XmlElem: THsJclSimpleXmlElem);
    procedure ParseItem(XmlElem: THsJclSimpleXmlElem; Definition: Boolean);
    procedure ParseItemDefinitionGroup(XmlElem: THsJclSimpleXmlElem);
    procedure ParseItemGroup(XmlElem: THsJclSimpleXmlElem);
    procedure ParseItemMetaData(XmlElem: THsJclSimpleXmlElem; ItemMetaData: TStrings);
    procedure ParseOnError(XmlElem: THsJclSimpleXMLElem; Target: THsJclMsBuildTarget);
    function ParseOtherwise(XmlElem: THsJclSimpleXmlElem; Skip: Boolean): Boolean;
    procedure ParseOutput(XmlElem: THsJclSimpleXMLElem; Task: THsJclMsBuildTask);
    procedure ParseParameter(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
    procedure ParseParameterGroup(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
    procedure ParseProject(XmlElem: THsJclSimpleXmlElem);
    procedure ParseProperty(XmlElem: THsJclSimpleXmlElem);
    procedure ParsePropertyGroup(XmlElem: THsJclSimpleXmlElem);
    procedure ParseTarget(XmlElem: THsJclSimpleXmlElem);
    procedure ParseTask(XmlElem: THsJclSimpleXMLElem; Target: THsJclMsBuildTarget);
    procedure ParseTaskBody(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
    procedure ParseUsingTask(XmlElem: THsJclSimpleXmlElem);
    function ParseWhen(XmlElem: THsJclSimpleXmlElem; Skip: Boolean): Boolean;
    procedure ParseXml(AXml: THsJclSimpleXML);
  protected
  ...
  public
    constructor Create(const AFileName: TFileName; AXml: THsJclSimpleXml; AOwnsXml: Boolean = False); overload;
    constructor Create(const AFileName: TFileName; Encoding: THsJclStringEncoding = seAuto; CodePage: Word = CP_ACP); overload;
    constructor Create(const AFileName: TFileName; ExtraImportsFileName: array of string; Encoding: THsJclStringEncoding = seAuto; CodePage: Word = CP_ACP); overload;
    destructor Destroy; override;
    ...
    function FindItemDefinition(const ItemName: string): THsJclMsBuildItem;
    function FindTarget(const TargetName: string): THsJclMsBuildTarget;
    ...
    property Xml: THsJclSimpleXml read FXml;
    property OwnsXml: Boolean read FOwnsXml write FOwnsXml;
    property Properties: THsJclMsBuildProperties read FProperties;
    property ItemCount: Integer read GetItemCount;
    property Items[Index: Integer]: THsJclMsBuildItem read GetItem;
    property ItemDefinitionCount: Integer read GetItemDefinitionCount;
    property ItemDefinitions[Index: Integer]: THsJclMsBuildItem read GetItemDefinition;
    property TargetCount: Integer read GetTargetCount;
    property Targets[Index: Integer]: THsJclMsBuildTarget read GetTarget;
    property UsingTaskCount: Integer read GetUsingTaskCount;
    property UsingTasks[Index: Integer]: THsJclMsBuildUsingTask read GetUsingTask;
    property ProjectExtensions: THsJclSimpleXMLElem read FProjectExtensions;
    ...
    property OnImport: THsJclMsBuildImportEvent read FOnImport write FOnImport;
    property OnToolsVersion: THsJclMsBuildToolsVersionEvent read FOnToolsVersion write FOnToolsVersion;
    property OnRegistryProperty: THsJclMsBuildRegistryPropertyEvent read FOnRegistryProperty write FOnRegistryProperty;
    property OnFunctionProperty: THsJclMsBuildFunctionPropertyEvent read FOnFunctionProperty write FOnFunctionProperty;
  end;
...
constructor THsJclMsBuildItem.Create;
begin
  inherited Create;
  FItemMetaData := TStringList.Create;
end;

destructor THsJclMsBuildItem.Destroy;
begin
  FItemMetaData.Free;
  inherited Destroy;
end;

//=== { THsJclMsBuildUsingTask } ===============================================

function THsJclMsBuildUsingTask.AddParameter(Parameter: THsJclMsBuildParameter): Integer;
begin
  Result := FParameters.Add(Parameter);
end;

constructor THsJclMsBuildUsingTask.Create;
begin
  inherited Create;
  FParameters := TObjectList.Create(True);
end;

destructor THsJclMsBuildUsingTask.Destroy;
begin
  FParameters.Free;
  inherited Destroy;
end;

function THsJclMsBuildUsingTask.GetParameter(
  Index: Integer): THsJclMsBuildParameter;
begin
  Result := THsJclMsBuildParameter(FParameters.Items[Index]);
end;

function THsJclMsBuildUsingTask.GetParameterCount: Integer;
begin
...
constructor THsJclMsBuildTask.Create;
begin
  inherited Create;
...
destructor THsJclMsBuildTask.Destroy;
begin
  FOutputs.Free;
  FParameters.Free;
  inherited Destroy;
...
function THsJclMsBuildTask.AddOutput(AOutput: THsJclMsBuildTaskOutput): Integer;
begin
  Result := FOutputs.Add(AOutput);
end;

function THsJclMsBuildTask.GetOutput(Index: Integer): THsJclMsBuildTaskOutput;
begin
  Result := THsJclMsBuildTaskOutput(FOutputs.Items[Index]);
end;

function THsJclMsBuildTask.GetOutputCount: Integer;
begin
...
constructor THsJclMsBuildTarget.Create;
begin
  inherited Create;
...
destructor THsJclMsBuildTarget.Destroy;
begin
  FErrorTargets.Free;
  FTasks.Free;
  FAfterTargets.Free;
...
function THsJclMsBuildTarget.AddTask(Task: THsJclMsBuildTask): Integer;
begin
  Result := FTasks.Add(Task);
end;

function THsJclMsBuildTarget.GetTask(Index: Integer): THsJclMsBuildTask;
begin
  Result := THsJclMsBuildTask(FTasks.Items[Index]);
end;

function THsJclMsBuildTarget.GetTaskCount: Integer;
begin
  Result := FTasks.Count;
end;

//=== { THsJclMsBuildProperties } ==============================================

constructor THsJclMsBuildProperties.Create(AParser: THsJclMsBuildParser);
begin
  inherited Create;
  FParser := AParser;
...
destructor THsJclMsBuildProperties.Destroy;
begin
...
procedure THsJclMsBuildProperties.Clear;
begin
...
procedure THsJclMsBuildProperties.Delete(Index: Integer);
begin
  //  - reserved properties defined by MsBuild,
  //      their value cannot be overriden or an error will be raised
  if (Index >= 0) and (Index < FReservedProperties.Count) then
    raise EHsJclMsBuildError.CreateRes(@RsEReservedProperty);
  ...
  raise EHsJclMsBuildError.CreateRes(@SRangeError);
end;

function THsJclMsBuildProperties.Get(Index: Integer): string;
begin
  //  - reserved properties defined by MsBuild,
  if (Index >= 0) and (Index < FReservedProperties.Count) then
  begin
...
function THsJclMsBuildProperties.GetCount: Integer;
begin
  Result := FReservedProperties.Count + FGlobalProperties.Count +
    FCustomProperties.Count + FEnvironmentProperties.Count;
end;

function THsJclMsBuildProperties.GetObject(Index: Integer): TObject;
begin
  //  - reserved properties defined by MsBuild,
  if (Index >= 0) and (Index < FReservedProperties.Count) then
  begin
...
function THsJclMsBuildProperties.GetRawValue(const Name: string): string;
var
  Index: Integer;
  XmlElem: THsJclSimpleXmlElem;
begin
  Index := IndexOfName(Name);
  XmlElem := nil;
  if Index >= 0 then
    XmlElem := THsJclSimpleXMLElem(Objects[Index]);
...
function THsJclMsBuildProperties.IndexOf(const S: string): Integer;
begin
  //  - reserved properties defined by MsBuild,
...
procedure THsJclMsBuildProperties.Insert(Index: Integer; const S: string);
begin
  //  - reserved properties defined by MsBuild,
  //      their value cannot be overriden or an error will be raised
  if (Index >= 0) and (Index < FReservedProperties.Count) then
...
procedure THsJclMsBuildProperties.MergeEnvironmentProperties(
...
procedure THsJclMsBuildProperties.Put(Index: Integer; const S: string);
begin
  //  - reserved properties defined by MsBuild,
  //      their value cannot be overriden or an error will be raised
  if (Index >= 0) and (Index < FReservedProperties.Count) then
...
procedure THsJclMsBuildProperties.PutObject(Index: Integer; AObject: TObject);
begin
  //  - reserved properties defined by MsBuild,
  if (Index >= 0) and (Index < FReservedProperties.Count) then
  begin
...
procedure THsJclMsBuildProperties.SetRawValue(const Name, Value: string);
var
  Index: Integer;
  XmlElem: THsJclSimpleXmlElem;
begin
  ...
  if Assigned(Parser.FFirstPropertyGroup) then
    ...
    raise EHsJclMsBuildError.CreateResFmt(@RsELocateXmlElem, [Name]);
end;

//=== { THsJclMsBuildParser } ==================================================

constructor THsJclMsBuildParser.Create(const AFileName: TFileName; Encoding: THsJclStringEncoding; CodePage: Word);
var
  AXml: THsJclSimpleXML;
begin
  AXml := THsJclSimpleXML.Create;
  try
...
constructor THsJclMsBuildParser.Create(const AFileName: TFileName; ExtraImportsFileName: array of string; Encoding: THsJclStringEncoding = seAuto; CodePage: Word = CP_ACP);
var
  I: Integer;
begin
...
constructor THsJclMsBuildParser.Create(const AFileName: TFileName;
  AXml: THsJclSimpleXml; AOwnsXml: Boolean);
begin
  inherited Create;
  FProjectFileName := AFileName;
  FXml := AXml;
  FOwnsXml := AOwnsXml;
  FProperties := THsJclMsBuildProperties.Create(Self);
...
destructor THsJclMsBuildParser.Destroy;
begin
...
procedure THsJclMsBuildParser.Clear;
begin
  ClearItems;
...
procedure THsJclMsBuildParser.ClearItemDefinitions;
begin
  FItemDefinitions.Clear;
end;

procedure THsJclMsBuildParser.ClearItems;
begin
  FItems.Clear;
end;

procedure THsJclMsBuildParser.ClearTargets;
begin
  FTargets.Clear;
end;

function THsJclMsBuildParser.EvaluateString(const S: string): string;
    ...
    repeat
      ...
      begin
        ...
        begin
          Position := Start;
          if not FindClosingBrace(Result, Position) then
            raise EHsJclMsBuildError.CreateResFmt(@RsEEndOfString, [Result]);
          PropertyName := Copy(Result, Start + 2, Position - Start - 2);
...
function THsJclMsBuildParser.EvaluateTransform(ItemList: TStrings; const Transform: string): string;
type
...
var
  ...
  procedure GetTransformParameters(Item: THsJclMsBuildItem; var Storage: TDynStringArray;
...
begin
  ...
  try
    ...
    begin
      GetTransformParameters(THsJclMsBuildItem(ItemList.Objects[Index]), TransformStorage, TransformParameters);
...
function THsJclMsBuildParser.EvaluateFunctionProperty(const Command: string): string;
begin
  if (not Assigned(FOnFunctionProperty) or not FOnFunctionProperty(Self, Command, Result)) and
     (not IgnoreFunctionProperties) then
    raise EHsJclMsBuildError.CreateResFmt(@RsEFunctionProperty, [Command]);
end;

function THsJclMsBuildParser.EvaluateList(const Name: string): string;
var
  Index: Integer;
  Transform: string;
  List: TStrings;
...
function THsJclMsBuildParser.EvaluateRegistryProperty(Root: HKEY; const Path, Name: string): string;
begin
  if (not Assigned(FOnRegistryProperty) or not FOnRegistryProperty(Self, Root, Path, Name, Result)) and
    ...
    raise EHsJclMsBuildError.CreateResFmt(@RsERegistryProperty, [RootKeyName(Root), Path, Name]);
end;

function THsJclMsBuildParser.FindItemDefinition(
  const ItemName: string): THsJclMsBuildItem;
var
  Index: Integer;
begin
  for Index := 0 to FItemDefinitions.Count - 1 do
  begin
    Result := THsJclMsBuildItem(FItemDefinitions.Items[Index]);
...
procedure THsJclMsBuildParser.FindItemIncludes(const ItemName: string; List: TStrings);
var
  Index: Integer;
  Item: THsJclMsBuildItem;
begin
  List.Clear;
  List.BeginUpdate;
  try
    ...
    begin
      Item := THsJclMsBuildItem(FItems.Items[Index]);
...
function THsJclMsBuildParser.FindTarget(const TargetName: string): THsJclMsBuildTarget;
var
  Index: Integer;
begin
  for Index := 0 to FTargets.Count - 1 do
  begin
    Result := THsJclMsBuildTarget(FTargets.Items[Index]);
...
class function THsJclMsBuildParser.SameItemName(const ItemName1, ItemName2: string): Boolean;
begin
  Result := SameText(ItemName1, ItemName2);
end;

function THsJclMsBuildParser.GetItem(Index: Integer): THsJclMsBuildItem;
begin
  Result := THsJclMsBuildItem(FItems.Items[Index]);
end;

function THsJclMsBuildParser.GetItemCount: Integer;
begin
  Result := FItems.Count;
end;

function THsJclMsBuildParser.GetItemDefinition(
  Index: Integer): THsJclMsBuildItem;
begin
  Result := THsJclMsBuildItem(FItemDefinitions.Items[Index]);
end;

function THsJclMsBuildParser.GetItemDefinitionCount: Integer;
begin
  Result := FItemDefinitions.Count;
end;

function THsJclMsBuildParser.GetPropertyValue(const Name: string): string;
begin
  Result := Properties.Values[Name];
end;

function THsJclMsBuildParser.GetTarget(Index: Integer): THsJclMsBuildTarget;
begin
  Result := THsJclMsBuildTarget(FTargets.Items[Index]);
end;

function THsJclMsBuildParser.GetTargetCount: Integer;
begin
  Result := FTargets.Count;
end;

function THsJclMsBuildParser.GetUsingTask(Index: Integer): THsJclMsBuildUsingTask;
begin
  Result := THsJclMsBuildUsingTask(FUsingTasks.Items[Index]);
end;

function THsJclMsBuildParser.GetUsingTaskCount: Integer;
begin
  Result := FUsingTasks.Count;
end;

procedure THsJclMsBuildParser.Init;
begin
  InitReservedProperties;
  InitEnvironmentProperties;
end;

procedure THsJclMsBuildParser.InitEnvironmentProperties;
begin
...
procedure THsJclMsBuildParser.InitReservedProperties;
var
  Index: Integer;
  Path: string;
...
begin
  ...
  try
    THsJclClrHost.GetClrVersions(DotNetVersions);
    ...
    begin
      ...
      raise EHsJclMsBuildError.CreateRes(@RsEMSBuildPath);
  finally
    DotNetVersions.Free;
  end;
...
procedure THsJclMsBuildParser.Parse;
begin
...
procedure THsJclMsBuildParser.ParseChoose(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXmlProp;
  SubElem: THsJclSimpleXmlElem;
  Executed, _Otherwise: Boolean;
begin
  for Index := 0 to XmlElem.PropertyCount - 1 do
  begin
    Prop := XmlElem.Properties.Item[Index];
    raise EHsJclMsBuildError.CreateResFmt(@RsEUnknownProperty, [Prop.Name]);
  end;
  ...
  begin
    ...
    begin
      if _Otherwise then
        raise EHsJclMsBuildError.CreateRes(@RsEMultipleOtherwise);
      _Otherwise := True;
      Executed := ParseOtherwise(SubElem, Executed);
    end
    else
    if not (SubElem is THsJclSimpleXMLElemComment) then
      raise EHsJclMsBuildError.CreateResFmt(@RsEUnknownElement, [SubElem.Name]);
  end;
end;

function THsJclMsBuildParser.ParseCondition(const Condition: string): Boolean;
var
...
function THsJclMsBuildParser.ParseConditionLength(const Condition: string; var Position: Integer; Len: Integer): Boolean;
type
  ...
  begin
    ...
    begin
      ...
      if Condition[Position] <> ')' then
        raise EHsJclMsBuildError.CreateResFmt(@RsEMissingParenthesis, [Condition]);
    ...
    begin
      ...
      begin
        ...
        case MiddleOperator of
          opUnknown:
            raise EHsJclMsBuildError.CreateResFmt(@RsEUnknownOperator, [Condition]);
          opAnd:
...
function THsJclMsBuildParser.ParseConditionOperand(const Condition: string; var Position: Integer; Len: Integer): Boolean;
type
...
function THsJclMsBuildParser.ParseConditionString(const Condition: string; var Position: Integer; Len: Integer): string;
var
...
begin
  ...
  begin
    ...
    while (Position <= Len) and (Condition[Position] <> NativeSingleQuote) do
      Inc(Position);
    if Position > Len then
      raise EHsJclMsBuildError.CreateResFmt(@RsEEndOfString, [Condition]);
    EndPos := Position;
    // skip closing quote
    Inc(Position);
  end
...
procedure THsJclMsBuildParser.ParseImport(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem, OldProjectExtensions: THsJclSimpleXmlElem;
  Condition: Boolean;
  Project: TFileName;
  SubXml: THsJclSimpleXml;
  SubOwnsXml: Boolean;
...
procedure THsJclMsBuildParser.ParseImportGroup(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseItem(XmlElem: THsJclSimpleXmlElem; Definition: Boolean);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseItemDefinitionGroup(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseItemGroup(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseItemMetaData(XmlElem: THsJclSimpleXmlElem; ItemMetaData: TStrings);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  Condition: Boolean;
...
procedure THsJclMsBuildParser.ParseOnError(XmlElem: THsJclSimpleXMLElem; Target: THsJclMsBuildTarget);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
function THsJclMsBuildParser.ParseOtherwise(XmlElem: THsJclSimpleXmlElem; Skip: Boolean): Boolean;
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseOutput(XmlElem: THsJclSimpleXMLElem; Task: THsJclMsBuildTask);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
  ...
  TaskOutput: THsJclMsBuildTaskOutput;
begin
  Condition := True;
  ...
  if TaskParameter = '' then
    raise EHsJclMsBuildError.CreateRes(@RsEMissingTaskParameter);

  if (PropertyName = '') and (ItemName = '') then
    raise EHsJclMsBuildError.CreateRes(@RsEMissingOutputName);

  if Condition then
  begin
    TaskOutput := THsJclMsBuildTaskOutput.Create;
...
procedure THsJclMsBuildParser.ParseParameter(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
  ...
  Parameter: THsJclMsBuildParameter;
begin
  Output := False;
  Required := False;
  ...
  Parameter := THsJclMsBuildParameter.Create;
...
procedure THsJclMsBuildParser.ParseParameterGroup(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseProject(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
  ...
  begin
    ...
    begin
      if Prop.Value <> 'http://schemas.microsoft.com/developer/msbuild/2003' then
        raise EHsJclMsBuildError.CreateResFmt(@RsEUnknownSchema, [Prop.Value]);
    end
    else
      raise EHsJclMsBuildError.CreateResFmt(@RsEUnknownProperty, [Prop.Name]);
  end;
  ...
  begin
    ...
    begin
      if Assigned(FProjectExtensions) then
        raise EHsJclMsBuildError.CreateRes(@RsEMultipleProjectExtensions);
...
procedure THsJclMsBuildParser.ParseProperty(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParsePropertyGroup(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXmlProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseTarget(XmlElem: THsJclSimpleXMLElem);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
begin
  ...
  if TargetName = '' then
    raise EHsJclMsBuildError.CreateRes(@RsEMissingTargetName);

  Target := THsJclMsBuildTarget.Create;
  FTargets.Add(Target);
...
procedure THsJclMsBuildParser.ParseTask(XmlElem: THsJclSimpleXMLElem; Target: THsJclMsBuildTarget);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseTaskBody(XmlElem: THsJclSimpleXMLElem; UsingTask: THsJclMsBuildUsingTask);
var
  Index: Integer;
  Prop: THsJclSimpleXMLProp;
  SubElem: THsJclSimpleXmlElem;
...
procedure THsJclMsBuildParser.ParseUsingTask(XmlElem: THsJclSimpleXmlElem);
var
  Index: Integer;
  Prop: THsJclSimpleXmlProp;
  SubElem: THsJclSimpleXmlElem;
  ...
  UsingTask: THsJclMsBuildUsingTask;
begin
  Condition := True;
  ...
  if TaskName = '' then
    raise EHsJclMsBuildError.CreateRes(@RsEMissingTaskName);

  if (AssemblyName = '') and (AssemblyFile = '') then
    raise EHsJclMsBuildError.CreateRes(@RsEMissingAssembly);

  if Condition then
  begin
    UsingTask := THsJclMsBuildUsingTask.Create;
...
function THsJclMsBuildParser.ParseWhen(XmlElem: THsJclSimpleXmlElem; Skip: Boolean): Boolean;
var
  Index: Integer;
  Prop: THsJclSimpleXmlProp;
  SubElem: THsJclSimpleXmlElem;
begin
  ...
  if XmlElem.PropertyCount <> 1 then
    raise EHsJclMsBuildError.CreateRes(@RsEConditionNotUnique);
...
procedure THsJclMsBuildParser.ParseXml(AXml: THsJclSimpleXML);
begin
  if AXml.Root.Name <> 'Project' then
    raise EHsJclMsBuildError.CreateResFmt(@RsENoProjectElem, [AXml.Root.Name]);
  ParseProject(AXml.Root);
end;

procedure THsJclMsBuildParser.Save;
begin
  Xml.SaveToFile(ProjectFileName);
end;

procedure THsJclMsBuildParser.SetPropertyValue(const Name, Value: string);
begin
  Properties.Values[Name] := Value;
end;

procedure THsJclMsBuildParser.XMLDecodeValue(Sender: TObject; var Value: string);
begin
  Value := XMLDecode(Value);
end;

procedure THsJclMsBuildParser.XMLEncodeValue(Sender: TObject; var Value: string);
begin
